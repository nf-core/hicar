process HOMER_FINDTADSANDLOOPS {
    tag "$meta.id"
    ...
    output:
    tuple val(meta), val(bin), path("${tagdir}/*.loop.2D.bed")  , emit: interactions
    tuple val(meta), val(bin), path("${tagdir}/*.tad.2D.bed")   , emit: tads
