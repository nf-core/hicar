process READS_SUMMARY {
    ...
    if(length(fs)>0){
        ...
        if(all(c("sample", "total", "samefrag", "duplicate", "non_duplicated", "trans", "cis", "longRange") %in% colnames(df))){
            df[, "sample"] <- sub(".stats.tsv", "", df[, "sample"])
            df[, "shortRange"] <- df[, "cis"] - df[, "longRange"]
            df[, "unmapped_multimapped"] <- df[, "total"] - df["samefrag"] - df[, "duplicate"] - df[, "non_duplicated"]
            df[, "unmapped_multimapped_rate"] <- 100*df[, "unmapped_multimapped"]/df[, "total"]
            df[, "samefrag_rat"] <- 100*df[, "samefrag"]/df[, "total"]
