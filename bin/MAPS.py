...
def init(p):
    ...
    for CHR1 in chroms:
        ...
        ps_short1 = pd.read_csv(parse_fname(CHR1, 'short', params),header=None,sep='\t')
        if peak_skip:
            continue
        for CHR2 in chroms:
            ...
            if CHR1!=CHR2:
                ps_short2 = pd.read_csv(parse_fname(CHR2, 'short', params),header=None,sep='\t')
            ...
            if ps_short.shape[0]:
                ...
                if ps_long.shape[0]:
                    ...
                    count_data_and = count_data[(count_data['bin1_mid'].isin(MACS2_peak_ranges_list_1)) & (count_data['bin2_mid'].isin(MACS2_peak_ranges_list_2))].copy()
                    if CHR1==CHR2:
                    ...
                    count_data_xor = count_data[(count_data.bin1_mid.isin(MACS2_peak_ranges_list_1)) ^ (count_data.bin2_mid.isin(MACS2_peak_ranges_list_2))]
                    if CHR1==CHR2:
                    ...
                    count_data_xor_bin1 = count_data_xor[(count_data_xor.bin1_mid.isin(MACS2_peak_ranges_list_1))].copy()
                    count_data_xor_bin1['1D_peak_bin1'] = 1
                    count_data_xor_bin1['1D_peak_bin2'] = 0
                    count_data_xor_bin2 = count_data_xor[(count_data_xor.bin2_mid.isin(MACS2_peak_ranges_list_2))].copy()
                    ...
                    qc_str = qc_str +\
                        'AND_set\t' + str(AND_sum) + '\tnumber of pairs in AND set at chromsome ' + CHR1 + ' and ' + CHR2 + '\n' +\
                        'XOR_set\t' + str(XOR_sum) + '\tnumber of pairs in XOR set at chromsome ' + CHR1 + ' and ' + CHR2 + '\n' +\
                        'NOT_set\t' + str(NOT_sum) + '\tnumber of pairs in NOT set at chromsome ' + CHR1 + ' and ' + CHR2 + '\n'
                    ...
                    print ('--saving output\n')
                    fout_name = params['OUT_DIR'] + 'reg_raw.' + str(CHR1) + '_' + str(CHR2) + '.' + params['DATASET_NAME'] + '.'+ str(int(params['BIN_SIZE']/1000)) + 'k.and'
                    reg_and.to_csv(fout_name, sep='\t')
                    fout_name = params['OUT_DIR'] + 'reg_raw.' + str(CHR1) + '_' + str(CHR2) + '.' + params['DATASET_NAME'] + '.'+ str(int(params['BIN_SIZE']/1000)) + 'k.xor'
                    reg_xor.to_csv(fout_name, sep='\t')
            else:
                print('no bin pairs in long or short bedpe files for chromosome ',CHR1, ' and ', CHR2, '. Doing next chromosome')
